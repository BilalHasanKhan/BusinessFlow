    ...
    public class CodeFirstMembershipProvider : CodeFirstExtendedProvider
            {
                ...
                public override string CreateAccount(string userName, string password, string email,out MembershipCreateStatus status, bool requireConfirmationToken)
                {
                ...
                public override bool ResetPasswordWithToken(string token, string newPassword)
                {
                    ...
                    using (BusinessFlowContext context = new BusinessFlowContext())
                    {
                        dynamic user = context.User.FirstOrDefault(Usr => Usr.PasswordVerificationToken == token && Usr.PasswordVerificationTokenExpirationDate > DateTime.UtcNow);
                        if (user != null)
                        {
                            dynamic newhashedPassword = CodeFirstCrypto.HashPassword(newPassword);
                            if (newhashedPassword.Length > 128)
                ...
                public override bool ExtendedValidateUser(string userNameOrEmail, string password)
                {
