...
namespace BusinessFlow.Models
{
    public static class CodeFirstSecurity
    {
        ...
        {
            CodeFirstExtendedProvider provider = (CodeFirstExtendedProvider)Membership.Provider;
            if (provider == null)
            {
        ...
        public static string CreateAccount(string userName, string password, string email,out MembershipCreateStatus status, bool requireConfirmationToken = false)
        {
            CodeFirstExtendedProvider provider = VerifyProvider();
            return provider.CreateAccount(userName, password, email, out status, requireConfirmationToken);
        }
